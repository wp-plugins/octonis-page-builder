 ...
 */
if(!function_exists('dateToTimestampOct')) {
    function dateToTimestampOct($date) {
        if(empty($a))
            return false;
        $a = explode(OCT_DATE_DL, $date);
...
if(!function_exists('importOct')) {
    function importOct($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassOct')) {
    function importClassOct($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), OCT_CODE) !== false) {
					$classFile = preg_replace('/'. OCT_CODE. '/i', '', $classFile);
				}
                $path = OCT_CLASSES_DIR. $classFile. '.php';
			}
            return importOct($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, OCT_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], OCT_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameOct')) {
    function toeGetClassNameOct($class) {
        $className = '';
		if(class_exists($class. strFirstUp(OCT_CODE))) 
			$className = $class. strFirstUp(OCT_CODE);
		else if(class_exists(OCT_CLASS_PREFIX. $class)) 
            $className = OCT_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjOct')) {
    function toeCreateObjOct($class, $params) {
        $className = toeGetClassNameOct($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectOct')) {
    function redirectOct($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsOct')) {
	function prepareParamsOct(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsOct::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeOct($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeOct')) {
	function prepareFieldCodeOct($string) {   
