<?php
abstract class viewOct extends baseObjectOct {
    /* 
     * @deprecated
     */
    protected $_tpl = OCT_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameOct::_()->getModule( $this->_code );
		$plTemplate = frameOct::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameOct::_()->getModule($plTemplate))
			$plTemplate = '';
	...
	public function getModel($code = '') {
		return frameOct::_()->getModule( $this->_code )->getController()->getModel($code);
	}
