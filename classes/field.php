...
class fieldOct {
	 ...
	 */
	public $adapt = array('htmlOct' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterOct::_($value, $this->adapt['dbFrom'], fieldAdapterOct::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlOct', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterOct::_($this, $this->adapt['html'], fieldAdapterOct::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameOct::_()->getModule('optionsOct')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', OCT_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsOct'] = $options;
				}
			}
			$htmlContent = htmlOct::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlOct::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterOct::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterOct::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsOct');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameOct::_()->getModule('optionsOct')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsOct'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlOct', $method)) {
			$output .= htmlOct::$method($name, $params);
			$output .= htmlOct::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', OCT_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', OCT_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', OCT_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterOct::_($id,'getExtraFieldOptions',fieldAdapterOct::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', OCT_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', OCT_LANG_CODE).':<br />';
					$output .= htmlOct::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', OCT_LANG_CODE).':<br />';
					$output .= htmlOct::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterOct::_($this, $this->adapt['htmlParams'], fieldAdapterOct::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', OCT_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', OCT_LANG_CODE).':<br />';
				$output .= fieldAdapterOct::_($params,'getFieldAttributes',  fieldAdapterOct::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsOct::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperOct');
		  $module = $this->checkVarFromParam($param,'moduleOct');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameOct::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameOct::_()->getModule($module)->isExternal())
		   $config_xml = frameOct::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = OCT_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', OCT_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameOct::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsOct'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlOct::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
