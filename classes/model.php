<?php
abstract class modelOct extends baseObjectOct {
	...
	public function getModule() {
		return frameOct::_()->getModule( $this->_code );
	}
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameOct::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				return true;
			} else 
	...
	public function delete($params = array()) {
		if(frameOct::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else 
			$this->pushError (__('Database error detected', OCT_LANG_CODE));
		return false;
	...
	public function insert($data) {
		...
		$id = frameOct::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameOct::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	...
	public function update($data, $where) {
		...
		if(frameOct::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameOct::_()->getTable( $this->_tbl )->getErrors());
		return false;
