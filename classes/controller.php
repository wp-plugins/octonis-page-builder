...
abstract class controllerOct {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameOct::_()->getModule( $this->getCode() );
		$className = '';
		if(importOct($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameOct($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importOct($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameOct($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(OCT_ADMIN),
				'remove' => array(OCT_ADMIN),
				'restore' => OCT_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - OCT_METHODS or OCT_USERLEVELS
	 */
	public function getPermissions() {
		return array();
	}
	public function getModule() {
		return frameOct::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqOct::getVar('page');
		$rowsLimit = (int) reqOct::getVar('rows');
		$orderBy = reqOct::getVar('sidx');
		$sortOrder = reqOct::getVar('sord');

		// Our custom search
		$search = reqOct::getVar('search');
		...
		// jqGrid search
		$isSearch = reqOct::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqOct::getVar('searchField'));
			$searchString = trim(reqOct::getVar('searchString'));
		...
		$res = dispatcherOct::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseOct();
		if($this->getModel()->removeGroup(reqOct::getVar('listIds', 'post'))) {
