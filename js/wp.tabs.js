	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('octWpTabs')) {
					$this.addClass('octWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.octTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.octTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.octTabContent');
					allTabsContent.hide();
